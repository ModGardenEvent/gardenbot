	...
	public static Response handleEventRegister(SlashCommandInteraction interaction) {
		...
		try {
			...
			if (currentEventResult.statusCode() != 200) {
				return new MessageResponse("There is no currently active event open for registration.")
						.markEphemeral();
			}
		} catch (Exception ex) {
		...
		if (guild != null && guild.getId().equals(GardenBot.DOTENV.get("GUILD_ID"))) {
			...
			if (guild.getMembersWithRoles(role).contains(interaction.event().getMember())) {
				return new MessageResponse("You are already a Mod Garden: Nature participant.")
						.markEphemeral();
			}
			guild.addRoleToMember(user, role).complete();
			return new MessageResponse("Successfully added you as a participant to Mod Garden: Nature.")
					.markEphemeral();
		}
