package net.modgarden.gardenbot.modal.account.link;
...
public class LinkModrinthModal extends Modal {
		...
		try {
			...
			if (stream.statusCode() < 200 || stream.statusCode() > 299) {
				...
				if (stream.statusCode() == 422) {
					return new EmbedResponse()
							.setTitle("Could not link your Modrinth account to your Mod Garden account.")
